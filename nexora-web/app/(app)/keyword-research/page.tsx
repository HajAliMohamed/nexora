...
export default function KeywordResearchPage() {
  ...
  const { data: results = [], isFetching, error } = useQuery({
    ...
    queryFn: () => apiFetch<KeywordResearchResult[]>(
      `/keyword-research/search?query=${encodeURIComponent(searchQuery)}&country=${country}&language=${language}`,
    ),
    enabled,
    retry: false,
  });
  ...
  const addMutation = useMutation({
    mutationFn: (data: { keyword: string; projectId: string; countryCode: string; languageCode: string }) =>
      apiFetch('/keyword-research/add-to-project', {
        method: 'POST',
  ...
  const handleSearch = useCallback(() => {
    if (searchQuery.trim()) setEnabled(true);
  }, [searchQuery]);
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-4">
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-muted-foreground mb-1">Requête</label>
              <Input
                value={searchQuery}
                onChange={e => { setSearchQuery(e.target.value); setEnabled(false); }}
      ...
      {is403 && (
        <div className="bg-destructive/10 text-destructive text-sm rounded-md p-3">
          Limite de recherches quotidienne atteinte.{' '}
          <a href="/billing" className="underline font-medium">Augmentez votre offre</a> pour plus de recherches.
        </div>
      )}
      ...
      {!isFetching && results.length === 0 && enabled && !is403 && (
        <p className="text-sm text-muted-foreground text-center py-8">Aucun résultat trouvé</p>
      )}
            ...
            <TableHeader>
              <TableRow>
                ...
                {(['volume', 'cpc', 'difficulty'] as const).map(col => (
                  ...
                  >
                    {col === 'volume' ? 'Volume' : col === 'cpc' ? 'CPC' : 'Difficulté'}
      ...
      <Dialog open={!!addModal} onClose={() => setAddModal(null)}>
        <DialogHeader>
          ...
          <DialogDescription>
            Mot-clé : <strong>{addModal?.keyword}</strong>
          </DialogDescription>
        </DialogHeader>
        <Select
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button
            onClick={() => {
              if (!selectedProject || !addModal) return;
              addMutation.mutate({
                keyword: addModal.keyword,
                projectId: selectedProject,
                countryCode: addModal.country,
                languageCode: addModal.language,
              });
            }}
