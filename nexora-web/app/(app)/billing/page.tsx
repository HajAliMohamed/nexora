...
export default function BillingPage() {
  ...
  const { data: usageData } = useQuery({
    ...
    refetchInterval: (q: any) => {
      const d = q.state.data as UsageResponse | undefined;
      return d?.usage ? 30000 : false;
    },
  });
  ...
  const portalMutation = useMutation({
    mutationFn: () =>
      apiFetch<{ url: string }>('/billing/create-portal-session', {
        method: 'POST',
      }),
    onSuccess: (data) => {
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          ...
          <Badge variant={currentPlan === 'free' ? 'secondary' : currentPlan === 'pro' ? 'default' : 'success'}>
            {currentPlan === 'free' ? 'Gratuit' : currentPlan === 'pro' ? 'Pro' : 'Agence'}
          </Badge>
        </CardHeader>
        <CardContent>
          {usage && limits && (
              ...
              <Card key={plan} className={`${isCurrent ? 'ring-2 ring-brand' : ''}`}>
                ...
                <CardContent>
                  <ul className="space-y-2 text-sm">
                    {PLAN_FEATURES.general.map(f => (
                      <li key={f.label} className="flex justify-between">
                        <span className="text-muted-foreground">{f.label}</span>
                        <span className="font-medium">{f[plan]}</span>
                      </li>
                    ))}
                  </ul>
                  <hr className="my-3" />
                  <ul className="space-y-2 text-sm">
                    {PLAN_FEATURES.data.map(f => (
                      <li key={f.label} className="flex justify-between">
                        <span className="text-muted-foreground">{f.label}</span>
                        <span className="font-medium">{f[plan]}</span>
                      </li>
                ...
                <CardFooter>
                  {isCurrent ? (
                    ...
                    <Button disabled variant="outline" className="w-full opacity-50 cursor-not-allowed">
                      Rétrograder (contacter le support)
                    </Button>
                  ) : (
                    <Button
                      ...
                      className="w-full"
                    >
                      {checkoutMutation.isPending ? 'Redirection...' : `Passer à ${plan === 'pro' ? 'Pro' : 'Agence'}`}
                    </Button>
                  )}
                </CardFooter>
              </Card>
