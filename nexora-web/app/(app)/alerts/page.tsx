...
function AlertCard({ alert, onMarkRead }: { alert: SeoAlert; onMarkRead: (id: string) => void }) {
  ...
  return (
    <Card
      className={`cursor-pointer transition-colors ${alert.readAt ? '' : 'border-brand/30 bg-brand/[0.02]'} hover:bg-muted/50`}
      onClick={() => !alert.readAt && onMarkRead(alert.id)}
    >
      <CardContent className="flex items-start gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          {isGain && (
            <p className="text-sm">
              Le mot-clé <strong>"{keyword}"</strong> est passé de la position{' '}
              {from} à la position {to}
              <Badge variant="success" className="ml-1">+{from - to}</Badge>
            </p>
          )}
          {isDrop && (
            <p className="text-sm">
              Le mot-clé <strong>"{keyword}"</strong> est tombé de la position{' '}
              {from} à la position {to}
              <Badge variant="destructive" className="ml-1">{to - from}</Badge>
            </p>
          )}
          {!isGain && !isDrop && (
            <p className="text-sm">{JSON.stringify(alert.payload)}</p>
          )}
...
export default function AlertsPage() {
  ...
  const { data: alerts = [] } = useQuery({
    ...
    refetchInterval: (q: any) => {
      const data = q.state.data as SeoAlert[] | undefined;
      return data?.some(a => !a.readAt) ? 30000 : false;
    },
  });
        ...
        <Card>
          <CardContent className="text-center py-12">
            <p className="text-sm text-muted-foreground">
              Pas encore d&apos;alerte. Elles apparaîtront ici lorsque vos positions changeront significativement.
            </p>
          </CardContent>
        </Card>
      )}
